...
Assume matrix C is initialized to zero. The `do_block` function is basic DGEMM
...
version of DGEMM repeatedly on matrices of size BLOCKSIZE by BLOCKSIZE.
BLOCKSIZE is called the blocking factor.

The function `do_block` is DGEMM with three new parameters si, sj, and sk to
...
void dgemm_basic_blocked(const uint32_t n, const double *A, const double *B,
                         double *C) {
